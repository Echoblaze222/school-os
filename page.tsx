// src/app/dashboard/counselor/meetings/page.tsx
...
export default async function CounselorMeetingsPage() {
  ...
  const isCounselor = await hasActiveAppointment(supabase, user.id, (profile as any).school_id, 'counselor')
  if (!isCounselor) redirect('/dashboard')
  ...
  return (
    <StaffMeetingsClient
      userId={user.id} schoolId={schoolId} role="counselor"
