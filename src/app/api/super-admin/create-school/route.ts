...
export async function POST(req: Request) {
  try {
    ...
    // ── Email, via the queue ────────────────────────────────────────────
    // Was a direct, unlogged getResend().emails.send() wrapped in a
    // try/catch that just console.error'd on failure - the principal's
    // access code, generated above, had no delivery record and no
    // retry if Resend was down or misconfigured (e.g. the sandbox
    // RESEND_FROM_EMAIL default, which 403s for any non-owner
    // recipient - see queueEmail.ts). queueEmail() logs every attempt
    // to email_deliveries and retries transient failures automatically
    // via the pg_cron job; no try/catch needed here anymore since it
    // never throws.
    ...
    await queueEmail({
      ...
      dedupeKey: `super-admin-create-school-${school.id}`,
      html: `
