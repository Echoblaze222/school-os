...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Secretary/create-user sets new non-principal users to 'stage_2_pending',

    // and principals to 'stage_1_pending'. Neither 'start' nor 'complete' is valid here.
    ...
    // Keep the stage as-is — the user still needs to complete onboarding.

    // stage_1_pending (principal) → goes to /onboarding/stage-1

    // stage_2_pending (staff/student) → goes to /onboarding/stage-2
    ...
    return NextResponse.json({
      ...
      email:            profile.email,   // ← client needs this to call signInWithPassword
