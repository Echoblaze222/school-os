    ...
    <Document>
      <Page size="A4" style={s.page}>
          ...
          {rows.length === 0 ? (
            ...
            rows.map((r, i) => (
              <View key={i} style={[s.tRow, i % 2 === 1 ? s.tRowAlt : {}]}>
                ...
                <Text style={[s.td, s.colScore]}>{r.score ?? 'N/A'}/{r.max}</Text>
        ...
        <View style={s.summary}>
          <View style={s.summaryBox}><Text style={s.summaryVal}>{data.avgPct !== null ? data.avgPct + '%' : 'N/A'}</Text><Text style={s.summaryLbl}>Average Score</Text></View>
          <View style={s.summaryBox}><Text style={s.summaryVal}>{data.attendancePct !== null ? data.attendancePct + '%' : 'N/A'}</Text><Text style={s.summaryLbl}>Attendance</Text></View>
        ...
        <View style={s.remarkBlock}>
          ...
          <Text style={s.remarkText}>{data.classTeacherRemark || 'N/A'}</Text>
        </View>
        <View style={s.remarkBlock}>
          <Text style={s.remarkTitle}>Principal's Remark</Text>
          <Text style={s.remarkText}>{data.principalRemark || 'N/A'}</Text>
        </View>
...
export async function POST(request: Request) {
  try {
    ...
    const docData = {
      ...
      studentName: rc.student?.full_name ?? 'N/A',
      admissionNo: rc.student?.admission_number ?? rc.student?.student_number ?? 'N/A',
      ...
      principalName: rc.approver?.full_name ?? 'N/A',
      classTeacherSignatureUrl: rc.class_teacher?.signature_url ?? null,
      classTeacherName: rc.class_teacher?.full_name ?? 'N/A',
