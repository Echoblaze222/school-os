...
export default function LeaderboardClient({ profile, school, userId }: Props) {
  ...
  return (
    <RolePageWrapper userId={userId} role="parent" profile={profile} school={school} title="Leaderboard">
      {loading ? <div className={styles.loading}><span/><span/><span/></div>
      : rows.length === 0
        ? <div className={styles.empty}><TrophyIcon size={40} color="var(--text-faint)" strokeWidth={1}/><p>No leaderboard data yet</p></div>
        : <div className={styles.list}>{rows.map((item:any) => (
            <div key={item.id} className={styles.card}>
              <div className={styles.cardIcon} style={{ background:sc+'20' }}>
                <TrophyIcon size={16} color={sc}/>
              </div>
