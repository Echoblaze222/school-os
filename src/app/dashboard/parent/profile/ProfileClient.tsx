...
export default function ProfileClient({ profile, school, userId }: Props) {
  ...
  async function uploadAvatar(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Folder-based path: {userId}/{timestamp}.ext
      // Makes RLS simple and keeps each user's files in their own folder
      ...
      if (dbErr) { setMsg('Photo uploaded but saving failed: ' + dbErr.message); return }
    ...
    <RoleSubHeader userId={userId} role="parent" profile={profile} school={school} title="My Profile" featureGroups={PARENT_FEATURE_GROUPS}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--space-5)', marginBottom: 'var(--space-7)' }}>
          ...
          <div style={{ width: 84, height: 84, borderRadius: '50%', background: sc, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', border: '3px solid ' + sc + '40' }}>
            ...
            {uploadingPhoto && (
              <div style={{ position: 'absolute', inset: 0, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ width: 24, height: 24, borderRadius: '50%', border: '3px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', animation: 'profile-spin 0.8s linear infinite' }} />
              </div>
            )}
          </div>
          <button
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', margin: 0, textTransform: 'capitalize' }}>{profile?.role}</p>
        </div>
      </div>
      ...
      <div style={{ background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-xl)', overflow: 'hidden', marginBottom: 'var(--space-5)' }}>
        ...
        {editing ? (
          <div style={{ padding: 'var(--space-5)', display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
            {[['Full Name', fullName, setFullName], ['Phone', phone, setPhone]].map(([label, val, setter]: any) => (
              <div key={label}>
                <label style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 6 }}>{label}</label>
                <input
                  value={val}
                  onChange={e => setter(e.target.value)}
                  style={{ width: '100%', height: 44, padding: '0 14px', background: 'var(--input-bg)', border: '1px solid var(--input-border)', borderRadius: 10, color: 'var(--text-primary)', fontSize: '0.875rem', outline: 'none' }}
                />
              </div>
            ))}
          ...
          <div>
            {fields.map(([label, value]) => (
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', padding: 'var(--space-3) var(--space-5)', borderBottom: '1px solid var(--glass-border)', fontSize: '0.85rem' }}>
