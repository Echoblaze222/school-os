...
export default function ScheduleClient({ profile, school, userId }: Props) {
  ...
  return (
    <RoleSubHeader userId={userId} role="coach" profile={profile} school={school} title="Schedule" featureGroups={COACH_FEATURE_GROUPS}>
      ...
      {showForm && (
        <div onClick={() => setShowForm(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-end', zIndex: 'var(--z-modal)' }}>
      ...
      {attendanceSession && (() => {
        ...
        return (
          <div onClick={() => setAttendanceSession(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-end', zIndex: 'var(--z-modal)' }}>
            <div onClick={e => e.stopPropagation()} className="glass-card" style={{ width: '100%', maxHeight: '88vh', overflowY: 'auto', padding: 20, borderRadius: '20px 20px 0 0' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
