...
function FilterPanel({
  filters, actionTypes, onApply, onClear,
}: {
  filters:     Filters
  actionTypes: string[]
  ...
  function handleClear() {
    ...
    setLocal(empty); onClear(); setOpen(false)
  }
        ...
        <div className={`${styles.filterPanel} animate-fade-up`}>
          <div className={styles.filterGrid}>
            <div className={styles.filterField}>
              ...
              <input type="date" className={`input ${styles.filterInput}`} value={local.from}
                max={local.to || undefined}
                onChange={e => setLocal(f => ({ ...f, from: e.target.value }))} />
            </div>
            <div className={styles.filterField}>
              <label className={styles.filterLabel}>To date</label>
              <input type="date" className={`input ${styles.filterInput}`} value={local.to}
                min={local.from || undefined}
                onChange={e => setLocal(f => ({ ...f, to: e.target.value }))} />
            </div>
            <div className={styles.filterField}>
              <label className={styles.filterLabel}>Action type</label>
              <select className={`input ${styles.filterInput}`} value={local.action}
            ...
            <div className={styles.filterField}>
              ...
              <input type="text" className={`input ${styles.filterInput}`} value={local.user}
...
export default function AuditClient({ entries, totalCount, page, actionTypes, filters, profile, school, userId }: Props) {
  ...
  function applyFilters(f: Filters) { startTransition(() => router.push(buildUrl(f, 0))) }
  function clearFilters() { startTransition(() => router.push(pathname)) }
  ...
  return (
    <RolePageWrapper userId={userId} role="teacher" profile={profile} school={school} title="Audit Log">
      ...
      {/* Filters */}
      <FilterPanel filters={filters} actionTypes={actionTypes} onApply={applyFilters} onClear={clearFilters} />
            ...
            {entries.map((entry, i) => {
              ...
              return (
                <div
                  ...
                  <div className={styles.entryStripe} style={{ background: ac.text }} aria-hidden />
                  <div className={styles.entryBody}>
                    <div className={styles.entryTopRow}>
                      <span className={styles.actionBadge} style={{ background: ac.bg, color: ac.text }}>
                    ...
                    <div className={styles.entryUserRow}>
                      <div className={styles.userAvatar} aria-hidden>{initials(entry.actor_name)}</div>
                      <div className={styles.userInfo}>
                        <span className={styles.userName}>{entry.actor_name}</span>
                        <span className={styles.userRole} style={{ background: rc.bg, color: rc.text }}>
                          {entry.actor_role.charAt(0).toUpperCase() + entry.actor_role.slice(1)}
                        </span>
                      </div>
                    </div>
                    {entry.details && <p className={styles.entryDetails}>{entry.details}</p>}
                  </div>
                </div>
              )
            })}
