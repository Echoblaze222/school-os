...
export default function QuizzesClient({ profile, school, userId }: Props) {
  ...
  const [aiDraftQuestions, setAiDraftQuestions] = useState<Question[] | null>(null)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const rawQuestions = draft.payload?.questions ?? []
      const converted: Question[] = rawQuestions.map((q: any) => {
        const options = (q.options ?? []).map((text: string, i: number) => ({ label: letters[i] ?? String(i), text }))
        const matchedLabel = options.find((o: any) => o.text === q.answer)?.label ?? options[0]?.label ?? 'A'
        return { question: q.question ?? '', options, answer: matchedLabel, marks: q.marks ?? 1 }
      })
      ...
      router.replace('/dashboard/teacher/quizzes')
    })()
  }, [searchParams])
  ...
  async function createQuiz() {
    ...
    if (data) {
      setNewQuiz(data)
      if (aiDraftQuestions?.length) {
        setQuestions(aiDraftQuestions)
        setAiDraftQuestions(null)
        setAiDraftBanner(null)
        if (aiDraftId) {
          supabase.from('ai_action_drafts').delete().eq('id', aiDraftId) // fire-and-forget cleanup
          setAiDraftId(null)
        }
      } else {
  ...
  if (step === 'create') return (
    <RolePageWrapper userId={userId} role="teacher" profile={profile} school={school} title="New Quiz" showBack={false}>
      ...
      {aiDraftBanner && (
        <div style={{ padding: '10px 14px', background: 'var(--brand-subtle)', border: `1px solid ${sc}44`,
