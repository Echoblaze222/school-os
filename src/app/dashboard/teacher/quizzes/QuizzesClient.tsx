  ...
  async function openPreview(quiz: any) {
    ...
    if (qs && qs.length > 0) {
      setQuestions(qs.map((q: any) => ({
        id: q.id,
        // FIX: read from both columns — older rows may only have `text`,
        // newer rows have both. q.question ?? q.text handles both cases.
  ...
  async function saveEditedQuestions() {
    ...
    const { error: insErr } = await supabase.from('quiz_questions').insert(
      questions.map((q, i) => ({
        ...
        text:     q.question, // FIX: `text` is NOT NULL — must always be sent
        question: q.question, // keep both in sync
        options:  q.options,
        answer:   q.answer,
        marks:    q.marks,
        position: i,
