...
export default function TimetableClient({ profile, school, userId }: Props) {
  ...
  const [form, setForm] = useState({ subject:'', room:'', start_time:'08:00', end_time:'09:00', class_level:'' })
  ...
  async function create() {
    ...
    await supabase.from('timetable').insert({
      ...form, day_of_week: day, school_id: school?.id, teacher_id: userId,
    })
    setForm({ subject:'', room:'', start_time:'08:00', end_time:'09:00', class_level:'' })
    setShowForm(false)
    load()
    setSaving(false)
  }
      ...
      {showForm && (
        <div style={{ background:'var(--glass-bg)', border:'1px solid var(--glass-border)', borderRadius:'var(--radius-xl)', padding:'var(--space-5)', marginBottom:'var(--space-5)' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'var(--space-3)' }}>
            {[
              { key:'subject',    label:'Subject *',   placeholder:'e.g. Mathematics', col:'1/-1' },
            ...
            {periods.map(p => (
              <div key={p.id} className={styles.periodCard}>
                ...
                <div className={styles.periodBody} style={{ borderLeftColor:sc }}>
                  ...
                  <p className={styles.periodMeta}>{p.class_level ? `${p.class_level} · ` : ''}{p.room ? `📍 ${p.room}` : ''}</p>
