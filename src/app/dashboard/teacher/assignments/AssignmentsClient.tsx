...
// FIXED: file attachment upload support, submission count via query
        ...
        <div style={{ background: 'var(--glass-bg)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-xl)', padding: 'var(--space-5)', marginBottom: 'var(--space-5)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
            ...
            <div style={{ gridColumn: '1/-1' }}>
              <input ref={fileRef} type="file" accept=".pdf,.doc,.docx,.ppt,.pptx,.txt" onChange={e => setAttachFile(e.target.files?.[0] ?? null)} style={{ display: 'none' }} />
              <button onClick={() => fileRef.current?.click()}
                style={{ width: '100%', height: 44, border: `1.5px dashed ${attachFile ? sc : 'var(--glass-border)'}`, borderRadius: 8, background: attachFile ? sc + '10' : 'transparent', color: attachFile ? sc : 'var(--text-muted)', fontWeight: 600, fontSize: '0.82rem', cursor: 'pointer' }}>
                {attachFile ? `📎 ${attachFile.name}` : '📎 Attach file (optional)'}
              </button>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-2)', marginTop: 'var(--space-4)' }}>
            <button onClick={createAssignment} disabled={saving || !form.title || !form.due_date || !form.class_id}
            ...
            <button onClick={() => { setShowForm(false); setAttachFile(null) }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 2, flexShrink: 0 }}>
                ...
                {item.file_url && (
                  <a href={item.file_url} target="_blank" rel="noreferrer"
                    style={{ fontSize: '0.68rem', color: sc, fontWeight: 700, textDecoration: 'none' }}>
                    View file
                  </a>
                )}
              </div>
