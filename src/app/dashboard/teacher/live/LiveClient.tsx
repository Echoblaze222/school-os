...
export default function LiveClient({ profile, school, userId }: Props) {
  ...
  async function load() {
    ...
    const { data, error: err } = await supabase
      .from('online_classes')
      .select('id, title, description, meeting_url, recording_url, is_live, scheduled_at, ended_at, created_at, class_id')
      .eq('school_id', school?.id)
      .eq('teacher_id', userId)
    ...
    <RolePageWrapper userId={userId} role="teacher" profile={profile} school={school} title="Live Classes">
      ...
      <div style={{ marginBottom: 'var(--space-3)', textAlign: 'right' }}>
        <button onClick={() => router.push('/dashboard/teacher/live/recordings')}
          className="pressable" style={{ background: 'none', border: 'none', color: sc, fontWeight: 600, fontSize: '0.8rem', cursor: 'pointer' }}>
          ▶ Recorded Classes
        </button>
      </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
            ...
            <div style={{ gridColumn: '1/-1', display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                placeholder="e.g. We'll cover photosynthesis"
                style={{ height: 40, padding: '0 12px', background: 'var(--input-bg)', border: '1px solid var(--input-border)', borderRadius: 8, color: 'var(--text-primary)', fontSize: '0.85rem', outline: 'none' }} />
            </div>
            ...
            <div style={{ gridColumn: '1/-1', display: 'flex', flexDirection: 'column', gap: 4 }}>
              <label style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--text-secondary)' }}>Meeting Link</label>
        ...
        : visibleSessions.length === 0
          ...
          : <div className={styles.list}>
            {visibleSessions.map(s => {
              ...
              const cls    = classLookup[s.class_id]
