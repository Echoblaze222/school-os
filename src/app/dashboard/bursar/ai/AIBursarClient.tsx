'use client'
// src/app/dashboard/bursar/ai/AIBursarClient.tsx
...
export default function AIBursarClient({ bursarName, schoolName, systemPrompt }: Props) {
  ...
  const firstName = bursarName.split(' ')[0]
  ...
  const S: React.CSSProperties = { maxWidth:'80%', backdropFilter:'blur(20px)' }
    ...
    <div style={{minHeight:'100dvh',background:'var(--bg-base)',display:'flex',flexDirection:'column',position:'relative',overflow:'hidden'}}>
      ...
      <header style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'var(--space-6) var(--space-5) var(--space-4)',position:'sticky',top:0,zIndex:'var(--z-card)',background:'var(--bg-overlay)',backdropFilter:'blur(20px)',borderBottom:'1px solid var(--glass-border)'}}>
        ...
        <div style={{flex:1,textAlign:'center',padding:'0 var(--space-3)'}}>
          ...
          <p style={{fontFamily:'var(--font-body)',fontSize:'0.68rem',color:'var(--text-muted)',letterSpacing:'0.05em',marginTop:2}}>Finance intelligence · {schoolName}</p>
        </div>
      ...
      <main style={{flex:1,overflowY:'auto',padding:'var(--space-4) var(--space-5)',display:'flex',flexDirection:'column',gap:'var(--space-4)',paddingBottom:200}}>
        {isEmpty&&<div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'var(--space-5)',paddingTop:'var(--space-8)',animation:'fade-up 0.5s ease'}}>
          ...
          <div style={{textAlign:'center'}}>
            ...
            <p style={{fontFamily:'var(--font-body)',fontSize:'0.85rem',color:'var(--text-secondary)',marginTop:6,lineHeight:1.5}}>Your finance assistant is ready.<br/>How can I help with the accounts?</p>
          </div>
      ...
      <nav className="bottom-nav" aria-label="Bursar navigation">
        ...
        <Link href="/dashboard/bursar/fees" className="nav-item"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg><span>Fees</span></Link>
        <Link href="/dashboard/bursar" className="nav-home" aria-label="Dashboard"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93a10 10 0 010 14.14M4.93 4.93a10 10 0 000 14.14"/></svg></Link>
        <Link href="/dashboard/bursar/payments" className="nav-item"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg><span>Payments</span></Link>
        <Link href="/dashboard/bursar/ai" className="nav-item active"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg><span>AI</span></Link>
      </nav>
    </div>
  )
}
