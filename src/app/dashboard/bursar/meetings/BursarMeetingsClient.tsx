...
// src/app/dashboard/bursar/meetings/BursarMeetingsClient.tsx

import { useRouter } from 'next/navigation'
import { useRealtimeTable } from '@/hooks/useRealtimeTable'
import RoleSubHeader from '@/components/RoleSubHeader'
import { BURSAR_FEATURE_GROUPS } from '../featureGroups'
import styles from './bursar-meetings.module.css'
...
export default function BursarMeetingsClient({
  userId, schoolId, meetings: initialMeetings, fetchError, profile, school,
}: Props) {
  ...
  const relevant = meetings.filter(m => m.target_audience === 'all_staff')
  ...
  return (
    <RoleSubHeader
      ...
      school={school}
      title="Meetings"
      featureGroups={BURSAR_FEATURE_GROUPS}
    >
    ...
    <div className={styles.listCard} style={{ animationDelay: `${index * 50}ms` }}>
      ...
      <div className={styles.listCardBody}>
        ...
        {!meeting.meeting_url && meeting.provider === 'livekit' && !isPast && meeting.is_live && (
          <button
            onClick={() => router.push(`/dashboard/bursar/meetings/room/${meeting.id}`)}
