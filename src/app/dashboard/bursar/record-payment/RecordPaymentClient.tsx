...
function fmtUSD(n: number, rate: number) {
  return new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2 }).format(n / rate)
}
...
export default function RecordPaymentClient({ bursarId, schoolInfo, usdRate, rateUpdatedAt }: Props) {
  ...
  async function pickStudent(s: StudentResult) {
    ...
    const { data } = await supabase
      ...
      .select('id, amount_due_ngn, amount_paid_ngn, balance_ngn, status, due_date, fee_structures(description, term, academic_year)')
      .eq('student_id', s.id)
            ...
            <div className={styles.field}>
                  ...
                  {(['NGN', 'USD'] as Currency[]).map(c => (
                    <button key={c} className={`${styles.currBtn} ${currency === c ? styles.currBtnActive : ''}`}
                      onClick={() => {
                        ...
                        const v = parseFloat(amount)
                        if (!isNaN(v) && v > 0) setAmount(c === 'USD' ? (v / usdRate).toFixed(2) : Math.round(v * usdRate).toString())
                        setCurrency(c)
                      }}>
                      {c === 'NGN' ? '₦ NGN' : '$ USD'}
                    </button>
              ...
              {amountNGN > 0 && (
                <div className={styles.convRow}>
                  ≈ {currency === 'NGN' ? fmtUSD(amountNGN, usdRate) : fmtNGN(amountNGN)} · Rate: ₦{usdRate.toLocaleString()}/$
                </div>
              )}
              <div className={styles.quickFill}>
                {[[0.25, '25%'], [0.5, '50%'], [1, 'Full']].map(([p, l]) => (
                  <button key={String(l)} className={styles.quickBtn}
                    onClick={() => {
                      const n = Math.round(invoice.balance_ngn * (p as number))
        ...
        {step === 4 && receipt && student && invoice && (
          <div className={`animate-scale-in ${styles.stepWrap}`}>
            ...
            <div className={`glass-card ${styles.rcpCard}`}>
              ...
              <div className={styles.rcpRows}>
                {[
                  ...
                  ['Term',        invoice.term],
                  ['Amount',      fmtNGN(amountNGN)],
                  ['USD',         fmtUSD(amountNGN, usdRate)],
                  ['Method',      method],
                  ['Reference',   reference || '—'],
            ...
            <div className={styles.rcpActions}>
              ...
              {receipt.pdfUrl && (
                <a href={receipt.pdfUrl} target="_blank" rel="noopener noreferrer" className="btn btn-ghost" style={{ flex: 1 }}>
                  Download PDF
                </a>
              )}
