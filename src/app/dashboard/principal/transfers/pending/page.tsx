...
export default async function PendingTransfersPage() {
  ...
  const { data: transfers } = await supabase
    ...
    .select(`
      id, student_id, initiated_at, notes, status,
  ...
  for (const t of transfers ?? []) {
    ...
    const outstanding = (sp?.fee_invoices ?? []).reduce(
      (s: number, f: any) => s + Math.max((f.amount_due ?? 0) - (f.amount_paid ?? 0), 0), 0
    )
    const { data: results } = await supabase
      .from('results').select('score, max_score').eq('student_id', t_.student_id)
    const rr = results ?? []
    const avg = rr.length > 0
      ? Math.round(rr.reduce((s: number, r: any) => s + (r.max_score > 0 ? (r.score / r.max_score) * 100 : 0), 0) / rr.length)
      : null
    rows.push({
      id: t_.id, student_id: t_.student_id,
      student_name:            sp?.full_name ?? 'Unknown',
      student_number:          sp?.student_number ?? null,
      origin_school_name:      sp?.schools?.name ?? null,
      destination_school_name: t_.destination?.name ?? null,
      status:                  t_.status ?? 'requested',
      initiated_at:            t_.initiated_at,
      notes:                   t_.notes ?? null,
      avg_score:               avg,
      total_results:           rr.length,
