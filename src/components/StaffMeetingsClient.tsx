...
// src/components/StaffMeetingsClient.tsx
//
...
//
// online_meetings.target_audience only has 4 real values in the schema
// (all_parents, all_teachers, all_staff, specific_class - see
// PrincipalMeetingsClient.tsx's own Audience type). There is no
// per-appointment audience (no "hostel_staff", no "exam_committee"), so
// every non-teacher-base role using this component is scoped to
// 'all_staff' only. If a role-specific audience value gets added to the
// schema later, extend audienceFilter per caller rather than widening this
// default for everyone.
//
// No create/edit capability here on purpose: none of the roles this was
// built for (hostel, vice-principal, ict, examination, counselor, nurse,
// librarian, coach) were confirmed to need meeting-scheduling rights, only
...
export default function StaffMeetingsClient({
  userId, schoolId, role, meetings: initialMeetings, fetchError, profile, school,
  audienceFilter = ['all_staff'],
}: Props) {
  ...
  const relevant = meetings.filter(m => audienceFilter.includes(m.target_audience))
      ...
      <div className={styles.listMain}>
        ...
        {relevant.length === 0 && !fetchError ? (
          <div className={styles.emptyState}>
            ...
            <p className={styles.emptyBody}>When the principal schedules a staff meeting, it will appear here.</p>
          </div>
        ) : (
          <>
            {upcoming.length > 0 && (
              <section className={styles.section}>
                ...
                <div className={styles.meetingList}>
                  {upcoming.map((m, i) => (
                    <MeetingCard key={m.id} meeting={m} index={i} userId={userId} schoolId={schoolId} role={role} />
                  ))}
                </div>
              </section>
            )}
            {past.length > 0 && (
              <section className={styles.section}>
                ...
                <div className={styles.meetingList}>
                  {past.map((m, i) => (
                    <MeetingCard key={m.id} meeting={m} index={i} isPast userId={userId} schoolId={schoolId} role={role} />
                  ))}
                </div>
              </section>
            )}
...
function MeetingCard({
  meeting, index, isPast = false, userId, schoolId, role,
}: {
      ...
      <div className={styles.listCardBody}>
        ...
        {meeting.meeting_url && !isPast && (
          <a
            ...
            onClick={() => logActivity({ userId, schoolId, type: 'meeting_joined', title: `Joined "${meeting.title}"`, href: `/dashboard/${role}/meetings` })}
          >
